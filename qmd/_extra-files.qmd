...
dir_vault <- clipr::read_clip() |> normalizePath("/", mustWork = FALSE)
```
...
for (i in ls() |> stringr::str_subset("^dir_")) {
  prettycheck:::assert_directory_exists(get(i))
}
```
...
for (i in ls() |> stringr::str_subset("^file_")) {
  prettycheck:::assert_file_exists(get(i))
}
```
