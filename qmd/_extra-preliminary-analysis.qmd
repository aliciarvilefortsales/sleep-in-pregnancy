...
# general_data |> 
#   stats_summary(
#     col = paste0("waso", "_", "sd"), 
#     threshold = NULL, # hms::parse_hm("12:00")
#     round_time =  TRUE
#   )

general_data |> stats_summary(paste0("awakenings", "_", "sd"))

# general_data |> stats_summary("age")
```
...
```{r}
general_data |> plot_dist(col = "waso_mean")
```
...
general_data |>
  plot_ggally(
    ...
    labels = c(
      "Duração do parto", "Idade", "Peso", "Altura", "Renda familiar"
    )
  )
```
...
general_data |>
  plot_ggally(
    ...
    labels = c("Duração do parto", "FPS", "WASO", "# Despertares")
  )
```
...
Tamanho de efeito para duraçõa do parto e FSP encontrado por @lee2004:
...
Tamanho de efeito para duração do parto e WASO encontrado por @lee2004:
